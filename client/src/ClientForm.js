...
function ClientForm({ onSave, onClose }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = {
        ...
        client_type: form.client_types.includes('Finance') ? 'financial' : form.client_types.length > 1 ? 'both' : 'insurance',
      };
  ...
  const s = {
    ...
    header: { backgroundColor: '#2B5C2B', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
    ...
    tab: (active) => ({ padding: '10px 16px', cursor: 'pointer', fontSize: '12px', fontWeight: active ? '700' : '400', color: active ? '#2B5C2B' : '#666', borderBottom: active ? '2px solid #2B5C2B' : 'none', backgroundColor: 'transparent', border: 'none', marginBottom: '-2px', whiteSpace: 'nowrap' }),
    ...
    sectionTitle: { fontSize: '12px', fontWeight: '700', color: '#2B5C2B', textTransform: 'uppercase', marginBottom: '10px', paddingBottom: '5px', borderBottom: '1px solid #E8F5E8', marginTop: '16px' },
    ...
    saveBtn: { padding: '8px 24px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '13px', backgroundColor: '#C9A227', color: 'white', fontWeight: '600' },
    checkboxRow: { display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' },
    typeCheckbox: (selected) => ({ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', borderRadius: '20px', border: `2px solid ${selected ? '#2B5C2B' : '#ddd'}`, backgroundColor: selected ? '#E8F5E8' : 'white', cursor: 'pointer', fontSize: '13px', fontWeight: selected ? '700' : '400', color: selected ? '#2B5C2B' : '#555' }),
  };

  const tabs = ['demographics', 'health', 'life', 'financial', 'notes'];
  const tabLabels = { demographics: '👤 Demographics', health: '🏥 Health Ins.', life: '💙 Life Ins.', financial: '💰 Financial', notes: '📝 Notes' };
      ...
      <div style={s.modal}>
        ...
        <form onSubmit={handleSubmit}>
            ...
            {activeTab === 'demographics' && (
              <div>
                ...
                <div style={s.checkboxRow}>
                  <input type="checkbox" name="is_married" checked={form.is_married} onChange={handleChange} id="married" />
                ...
                <div style={s.checkboxRow}>
                  <input type="checkbox" name="mailing_different" checked={form.mailing_different} onChange={handleChange} id="mailing" />
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '12px' }}>
                  {['Health', 'Life', 'Finance'].map(type => (
                    <div key={type} style={s.typeCheckbox(form.client_types.includes(type))} onClick={() => handleClientTypeChange(type)}>
            ...
            {activeTab === 'financial' && (
              <div>
                ...
                {form.plan_start_date && (
                  <div style={{ backgroundColor: '#FDF8E8', border: '1px solid #C9A227', borderRadius: '6px', padding: '10px', marginTop: '8px' }}>
                    <p style={{ margin: 0, fontSize: '13px', color: '#856404' }}>
                      📅 An 11-month reminder will be set for {new Date(new Date(form.plan_start_date).setMonth(new Date(form.plan_start_date).getMonth() + 11)).toLocaleDateString()}
                    </p>
                  </div>
                )}
          ...
          <div style={s.footer}>
            ...
            <button type="submit" style={s.saveBtn} disabled={saving}>{saving ? 'Saving...' : 'Save Client'}</button>
          </div>
        </form>
      </div>
    </div>
